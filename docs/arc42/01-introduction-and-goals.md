...
Access is available through three first-class channels: a web UI at `start.operaton.org`, the `npx operaton-starter` CLI, and a single `curl` command — all backed by the same REST API (`POST /api/v1/generate`).
...
| 3 | **Channel Consistency** | Web UI, CLI, and REST API invoke the same engine — identical output guaranteed |
...
Integrates the REST API into an internal Backstage developer portal. Self-hosts the Docker image with enterprise defaults via environment variables.
