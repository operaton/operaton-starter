...
- [ ] Task 3: Commit seed `.operaton-starter.yml` to `kthoms/operaton-examples` (external repo — coordinate separately)
...
- [ ] Task 5: Verify `starter.examples.repositories=kthoms/operaton-examples` is present in `application.properties` (set in Story 8.2)
...
- The `kthoms/operaton-examples` repository is an external GitHub repository — the seed manifest commit is a separate action from the operaton-starter PR.
